...
export default function LoginPage() {
  ...
  return (
    <div className="max-w-md mx-auto space-y-8">
      <section className="space-y-3">
        ...
        <form onSubmit={onSwitch} className="space-y-2">
          ...
          <input
            ...
            autoFocus
          />
          <button type="submit" disabled={!canSwitch || busy} className="bg-primary text-primary-foreground px-4 py-2 rounded disabled:opacity-50">
            Sign in
          </button>
        </form>
      </section>
      ...
      <section className="space-y-3 border-t border-border pt-6">
        ...
        <form onSubmit={onCreate} className="space-y-2">
          ...
          <input
            ...
            placeholder="newuser"
          />
          <button type="submit" disabled={!canCreate || busy} className="bg-primary text-primary-foreground px-4 py-2 rounded disabled:opacity-50">
            Create
          </button>
        </form>
      </section>
      ...
      {error && (
        <div role="alert" className="border border-destructive bg-destructive/10 text-destructive px-3 py-2 rounded text-sm">
          {error}
        </div>
      )}
    </div>
