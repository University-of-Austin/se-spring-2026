...
export default function PostCard({ post, pending = false }: Props) {
  ...
  return (
    <article className={`border border-border rounded-lg bg-card px-4 py-3 ${pending ? "opacity-60" : ""}`}>
      <header className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        {pending && <span className="ml-auto text-xs italic text-muted-foreground">posting…</span>}
      </header>
