...
**X-Username and auth:** The X-Username header is identity metadata, not authentication. The server trusts whatever value the client sends, so anyone can claim any username. Real authentication would verify the claim instead of trusting it: a login endpoint returns a signed token, the client re-sends that token on each protected request, and the server checks the signature before acting. That approach buys tamper-resistance, expiration, and revocation.
